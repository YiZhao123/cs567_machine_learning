...
def compute_distances(Xtrain, X):
	...
	for i in range(lentest):
		...
		for j in range(lentrain):
			Xtexti = X[i,:]
			Xtrainjd = Xtrain[j,:]
			onearray.append(np.linalg.norm(X[i] - Xtrain[j]))

		resultmatrix.append(onearray)

	dists = np.array(resultmatrix)
...
def predict_labels(k, ytrain, dists):
	...
	for i in range(len(dists)):
		...
		onetestdic = {}
		onetestalldist = []
		for j in range(len(onetestdis)):
			onetestalldist.append(onetestdis[j]) #list save all distance#
			if onetestdis[j] not in onetestdic.keys():
				onetestdic[onetestdis[j]] = [ytrain[j]]
			else:
				onetestdic[onetestdis[j]].append(ytrain[j])#dict: key is distance value is list saving labels#
		onetestalldist.sort()
		count = 0
		tempdict = {}
		for m in range(len(onetestalldist)):
			value = onetestdic[onetestalldist[m]] #value is a list saving labels#
			value.sort()
			for index in range(len(value)):
				if value[index] not in tempdict.keys():
					tempdict[value[index]] = 1
				else:
					tempdict[value[index]] = tempdict[value[index]] + 1
				count += 1
				if count == k:
					break
			if count == k:
		...
		maxnumber = 0
		maxlabel = -1
		for key in tempdict:    #key is the label and value is the appear times in k#
			if tempdict[key] > maxnumber:
				maxnumber = tempdict[key]
				maxlabel = key
			elif tempdict[key] == maxnumber and key < maxlabel:
				maxlabel = key

		ytestresult.append(maxlabel)


	ypred = np.array(ytestresult)
	return ypred
