...
def logistic_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    templist= w.tolist()
    templist.append(b)
    neww = numpy.array(templist)
    dim = len(Xtrain[0])

    for iterationsnum in range(max_iterations):
        sum = numpy.mat(numpy.zeros(dim+1))
        for i in range(len(Xtrain)):
            onextrain = []
            for d in range(len(Xtrain[i])):
                onextrain.append(Xtrain[i][d])
            onextrain.append(1)

            expmatrix = numpy.mat(neww) * (numpy.mat(onextrain).T)
            expreal = expmatrix[0,0]
            x = numpy.exp(expreal)
            if x == numpy.inf:
                exp = 10**50/(1+10**50)
            else:
            ...
            hehe = exp - ytrain[i]
            heheagain = hehe * numpy.mat(onextrain)
            sum = sum + heheagain

        sum = sum / (len(Xtrain)*1.0)
        neww = neww - step_size*sum

    w = numpy.array(neww[0, 0:dim])
    b = neww[0, -1]
...
def logistic_test(Xtest, ytest, w, b, t = 0.5):
    ...
    for i in range(len(ytest)):
        ...
        for d in range(len(onetest)):
            exper += w[0, d]*onetest[d]

        x = numpy.exp(exper)
        if x == numpy.inf:
            guessy = 10**50/(1+10**50)
        else:
            guessy = x*1.0/(1+x)
    ...
    test_acc = accurenum*1.0/totalnum
...
def feature_square(Xtrain, Xtest):
    ...
    for i in range(len(Xtrain)):
        temp = []
        temp.append(numpy.square(Xtrain[i][0]))
        temp.append(numpy.square(Xtrain[i][1]))
        Xtrain_s.append(temp)

    Xtest_s = []
    for i in range(len(Xtest)):
        temp = []
        temp.append(numpy.square(Xtest[i][0]))
        temp.append(numpy.square(Xtest[i][1]))
        Xtest_s.append(temp)
