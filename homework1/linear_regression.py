...
def linear_regression_noreg(X, y):
  ...
  xmatrix = np.mat(X)
  xmatrixT = xmatrix.T
  xTxInver = (xmatrixT*xmatrix).I
  ymatrix = np.mat(y).T
  result = xTxInver*xmatrixT*ymatrix
  templist = []
...
def regularized_linear_regression(X, y, lambd):
  ...
  xmatrixT = xmatrix.T
  d = len(xmatrixT)-1
  idenmatrix = np.matrix(np.identity(d+1))
  lamidenmatrix = lambd*idenmatrix
  xTxInver = (xmatrixT*xmatrix + lamidenmatrix).I
  ymatrix = np.mat(y).T
  result = xTxInver*xmatrixT*ymatrix
  templist = []
  for i in range(len(result)):
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  #				 YOUR CODE HERE					#
  #####################################################
  xmatrix = np.mat(Xtrain)
  xmatrixT = xmatrix.T
  ymatrix = np.mat(ytrain).T
  ...
  for lambd in lambds:
    lamidenmatrix = lambd*idenmatrix
    xTxInver = (xmatrixT*xmatrix + lamidenmatrix).I
    resultw = xTxInver*xmatrixT*ymatrix

    yvalget = Xval*resultw
    rsssum = 0
    for i in range(N):
      get = yvalget[i,0]
      target = yval[i]
      rsssum += np.square(get-target)
