...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output = np.dot(X, self.params['W'])
        N, D = forward_output.shape
        newB = np.zeros((N, D))
        for n in range(N):
            newB[n] = self.params['b']
    ...
    def backward(self, X, grad):
        ...
        m, D = self.gradient['b'].shape
        N, d = X.shape
        temp = np.ones((1, N))
        self.gradient['b'] = np.dot(temp, grad)
...
class relu:
    ...
    """
        a = X.shape
        forward_output = np.zeros(a)
        length = len(a)
        if length == 2:
            for i in range(a[0]):
                for j in range(a[1]):
                    if X[i, j] > 0:
                        forward_output[i, j] = X[i, j]
        elif length == 4:
            for i in range(a[0]):
                for j in range(a[1]):
                    for k in range(a[2]):
                        for l in range(a[3]):
                            if X[i, j, k, l] > 0:
                                forward_output[i, j, k, l] = X[i, j, k, l]
    """
            ...
            for i in range(a[0]):
                for j in range(a[1]):
                    for k in range(a[2]):
                        for l in range(a[3]):
                            if X[i, j, k, l] > 0:
                                backward_output[i, j, k, l] = grad[i, j, k, l]
        """
