...
def pca(X = np.array([]), initial_dims = 50):
    ...
    XXT = np.matmul(np.transpose(mean_removed),mean_removed)

    eig_vals, eig_vects = np.linalg.eig(XXT)
    eig_val_index = np.argsort(eig_vals)

    eig_val_index = eig_val_index[:-(initial_dims + 1): -1]
    reg_eig_vects = eig_vects[:, eig_val_index]
    
    reg_eig_vects /= np.linalg.norm(reg_eig_vects,axis=0)
    #print np.linalg.norm(reg_eig_vects)
